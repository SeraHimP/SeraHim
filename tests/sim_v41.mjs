...
// ==================== 四、闪电杖：67% 无视防御 + 满充重伤 + 切目标严格归零 ====================
{
  const P = SkillLibrary.weapon_lightning.defaultParams;
  T('无视防御上限 67%（v43 定稿，原 90%）', P.maxPenPct === 67);
...
// 用户："你这做的植被都跑到了贴图底下，正常根本看不到。"
{
  ...
  // 2026-09-04：风格化 demo（visualStyle==='stylized'，见 Config.stylizedPalettes 头注）
  ...
  // v58.5：召唤师峡谷（jungleMode）改成只摆灌木/岩石、不摆树（用户："都是低矮的
  // 灌木丛和零星的石头"），把原来单一的"树/岩/灌木三选一"摆放判据拆成了
  // jungleMode（灌木/岩石两支）与非 jungleMode（树/岩/灌木三支）——push 调用总数
  // 从 3 变成 5，但每一支都仍然用同一个 y0（本条断言真正要守的是"不各自另算
  // 一个高度"，不是数字 3 本身）。
  T('植被摆放（含 jungleMode 拆出的分支）都用同一个高度 y0，没有另算一份',
    (veg.match(/push\(\[x, y0, y,/g) || []).length === 5);
