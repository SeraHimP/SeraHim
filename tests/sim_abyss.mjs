...
lw.nextWaveTime=99999; lw.waveNumber=2; spawned=[]; lw._enqueueSpawn({faction:'blue',direction:'forward',targetFactions:['red']},mapSys.currentMap.lanes[0]);
...
lw.waveNumber=3; spawned=[]; lw._enqueueSpawn({faction:'blue',direction:'forward',targetFactions:['red']},mapSys.currentMap.lanes[0]);
for(let i=0;i<30;i++){lw.update(0.5);lw.nextWaveTime=99999;}
T(`炮兵波含炮兵1（实际 ${spawned.join('/')}）`, _cnt(spawned,'siege')===1);
{
  lw.waveNumber=4; const s4=[]; const _bak=spawned; spawned=s4;
  lw._enqueueSpawn({faction:'blue',direction:'forward',targetFactions:['red']},mapSys.currentMap.lanes[0]);
  for(let i=0;i<30;i++){lw.update(0.5);lw.nextWaveTime=99999;}
...
lw.waveNumber=6; spawned=[]; lw._enqueueSpawn({faction:'blue',direction:'forward',targetFactions:['red']},mapSys.currentMap.lanes[0]);
for(let i=0;i<30;i++){lw.update(0.5);lw.nextWaveTime=99999;}
