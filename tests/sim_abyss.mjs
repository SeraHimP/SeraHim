...
// v34（Q1）：水晶塔前移到高地入口（距入口精确180）+ 枢纽塔收角 → 两者射程圈【衔接重叠】，
// 高地防线不再有真空带（旧断言"完全分开"是旧布局的产物）。改验：仍保有纵深（>250）且确实衔接（<360）。
T('v34 枢纽塔↔水晶塔 射程衔接有纵深(250~360)', bHQ.every(h=>{const d=dist(h,bBase);return d>250&&d<360;}));
