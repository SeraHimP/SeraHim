...
// ==================== 四、属性面板 ====================
{
  ...
  // （RELATED_STATS.attackDamage），塔和小兵谁都不再单独占这一格。
  T('板②b-子弹速度已从网格里删除，塔和小兵都不再单独显示', (() => {
    const a = keysOf('tower'), b = keysOf('minion');
    return !a.includes('bulletSpeed') && !b.includes('bulletSpeed');
  })());
  T('板③-塔与兵**共用同一张表**（子弹速度移出网格后两边形状完全一致）', (() => {
    const a = keysOf('tower'), b = keysOf('minion');
