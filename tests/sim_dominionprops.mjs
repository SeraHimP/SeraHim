...
// ==================== 一、build() 基本产出 ====================
{
  ...
  T('②-建出了不少几何体（5 个功能区+7 面旗+2 处枢纽旗杆+地形装饰，量级应远超几十个）',
    layer.group.children.length > 80);
...
// ==================== 六、地形装饰必须遵守 isWalkable 门控 ====================
{
  ...
  const countNoAccent = layer1.group.children.length;

  const scene2 = makeScene();
  const layer2 = new DominionPropsLayer(scene2);
  layer2.setWalkableFn(ringIsWalkable);
  layer2.build({ currentMap: { ...map, id: 'dominion_test_v1_ring' } });
  const countWithAccent = layer2.group.children.length;

  T('⑮-地形装饰只会摆在不可走的地方：整张图全部可走时，几何数量明显少于有不可走区域时',
    countNoAccent < countWithAccent);

  const scene3 = makeScene();
  const layer3 = new DominionPropsLayer(scene3);
  // 不调用 setWalkableFn：没有可走判定时，地形装饰整段跳过（宁可不摆，不能摆错）。
  layer3.build({ currentMap: { ...map, id: 'dominion_test_v1_nowalkfn' } });
  T('⑯-没有注入 isWalkable 时，地形装饰整段跳过（跟 HowlingAbyssDecor 同一条底线）',
    layer3.group.children.length === countNoAccent);
}
...
// ==================== 七、setShadowLevel / clear ====================
{
  ...
  const meshes = layer.group.children.filter((o) => o.isMesh);
  T('⑰-setShadowLevel 打开阴影后，所有网格都开启 castShadow/receiveShadow',
