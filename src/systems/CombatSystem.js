...
export class CombatSystem {
  ...
  selectTarget(tower, minions) {
    ...
    const getPriority = (m) => {
      // 统治战场·水晶之痕 第五轮：用户定稿"中立据点会优先攻击正在占领该据点
      // 的阵营的单位"——谁把 targetId 指向这座中立据点自己（正在占它），
      // 中立据点就优先反过来打谁。只对"据点自己 + 还是中立态"生效（不影响
      // 普通塔，也不影响已经被占领的据点——已占领的据点认定敌我已经跟占领
      // 无关，走下面通用的优先级表就够了），排在比工程兵保护还高的档位：
      // 中立据点的生死直接系在"谁在占我"上，比"救工程兵"更紧迫。
      if (tower.isCapturePoint && tower._mapFaction === FACTIONS.NEUTRAL && m.targetId === tower.id) return 8;
