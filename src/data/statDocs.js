...
export const STAT_DOCS = {
  ...
  // v51.12：Q7——用户"攻速点开的窗口，描述为：每秒攻击的次数"。这条 doc 现在
  // 是"攻速"这一格（statPanelLayout 的 data-stat 仍指向 bonusAttackSpeedPct 这个
  // 键，没改）真正弹出的说明：desc 讲"攻速"本身是什么，label 则只在它作为
  // 【攻速加成】关联属性行挂到别处时才会被用到（见 UIManager._showStatDoc 对这个
  // key 的特判——弹窗主体的标题/数值改成显示算完的实际攻速，不再是这里的原始
  // 百分比）。
  bonusAttackSpeedPct: {
    label: '攻速加成',
    desc: '每秒攻击的次数。',
    formula: '**面板值不等于实际值**：有效加成 = 正值 × 攻击速度收益率(默认 0.667) + 负值。'
  ...
  attackSpeedRatio: {
    ...
    formula: '见「攻速加成%」。收益率越低，堆攻速的边际收益越差。',
  },
  attackRange: {
    label: '攻击距离',
    desc: '能够攻击到目标的最大距离。',
  ...
  // v51.27（Q5）：用户"添加属性窗口里'splashRadius'不知道是啥，汉化"——之前没有
  // FIELD_META/STAT_DOCS 条目，各处的中文标签查找都落进"没有就显示原始英文键名"的
  // 兜底分支，直接把字段名原样露了出来。
  splashRadius: {
    label: '溅射半径',
    desc: '普通攻击命中时，对目标周围半径内的其它敌人额外造成一份溅射伤害。0 = 无溅射。',
    formula: '溅射伤害与主目标同一次结算，各吃各自目标的抗性/护盾；攻城车攻城模式下'
           + '半径改由模式本身给出，取两者较大值。',
  },
  armor: {
    label: '护甲',
  ...
  // v51.9：新增，与固定护盾的区别是"打没了就没了，不会自动回满"——
  // 模板里配置这个值，出生即赠送一份这样的护盾（factories.js 的 grantTemplatePlainShield）。
  plainShieldFlat: {
    label: '护盾',
    desc: '出生即获得的一份护盾，打没了就没了，不会自动回复（与固定护盾的关键区别）。',
    formula: '承伤顺序：临时护盾 → 固定护盾 → 护盾 → 生命值。',
  },
  ...
  // v51.9：新增，远古之力从全属性加成改用这个（用户："远古之力的属性改为【核心
  // 属性加成】+2.5%"）。与全属性加成同一套乘算方式，只是放大的范围窄很多。
  coreStatsPct: {
    label: '核心属性加成%',
    desc: '只放大攻击力、法术强度、护甲、魔法抗性、攻速、移速这六项核心战斗属性。',
    formula: '作用在这六项基础属性上，与全属性加成是两条独立的乘算通路，互不影响。',
  },
  damageAmpPct: {
    label: '伤害增幅%',
    desc: '放大自己造成的全部伤害。',
  ...
  manaRegen: {
    label: '法力回复/秒',
    desc: '每秒自动回复的法力（实际值）。',
    formula: '实际回复 = 法力恢复(属性) × 基础法力恢复 × (1+法力获取加成%)。'
           + '同样受"没装主动技能就恒为0"这条规则约束。',
  },
  manaStart: { label: '出场法力', desc: '单位出场时的初始法力。', formula: '不超过最大法力。' },
  manaFloor: { label: '法力下限', desc: '释放主动技能后法力回落到的值。', formula: '默认 0（清空）。' },
  // v51.27（Q5）：新增——与 baseHealthRegenMod 同一形状，只乘在法力的【被动回复】上。
  baseManaRegenMod: {
    label: '基础法力恢复',
    desc: '对该单位【法力回复】的修正系数，百分比，默认 100%。只作用于被动回复，'
         + '不影响攻击/受击获得的法力（那部分走法力获取加成%）。',
  },
  // v51.27（Q5）：原 manaGainPct，面板显示名改叫"法力获取加成"（用户嫌"法力获得
  // 效率"不好听）——字段名本身不变，见 fields.js 里的说明。
  manaGainPct: {
    label: '法力获取加成%',
    desc: '该单位从任意渠道（被动回复/攻击/受击）获得的法力值再乘这个百分比，默认 0。',
    formula: '负值会打折（如嚎哭深渊光环 -50%），不会打到负数法力。',
  },
