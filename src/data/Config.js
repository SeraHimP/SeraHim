export const CONFIG = {
  ...
  ui: {
    ...
    weatherFx: {
      enabled: true,
      maxRain: 1400,      // 满充能时的雨滴数（盒子随缩放变大，密度自动跟着走）
      maxSnow: 900,
      maxDust: 260,       // 晴天的暖色浮尘。晴要是什么都不画，切到晴就像"天气关了"
      ceiling: 520,       // 粒子天花板高度（世界单位）
      viewPad: 1.15,      // 粒子盒相对可见范围的放大系数（留一点余量防边缘露空）
      rainSpeed: 1500, rainLen: 46, rainAlpha: 0.50,
      rainTilt: 0.35,     // 无风时的固有倾斜（垂直下落看着像静态噪点）
      rainTiltWind: 0.75, // 风充能满时额外增加的倾斜
      snowSpeed: 130, snowSize: 7, snowAlpha: 0.70, snowSway: 28, snowWindPush: 1.4,
      dustSpeed: 18, dustSize: 7, dustAlpha: 0.30,
      // 雾是一张贴在单位上方的薄纱片，不是 scene.fog（见 WeatherLayer 头注）。
      // alpha 是实拍调下来的：0.42 时满充能会把整屏刷成灰白，战场完全读不出来
      //（连地图外的空白都一起糊）。0.20 是"看得出起雾了、但还看得清打成什么样"。
      fogY: 110, fogAlpha: 0.20
    },
    crystal: {
