export const CONFIG = {
  ...
  ui: {
    ...
    rangeRing: {
      ...
      fadeOuter: 50,        // 射程 + 这么多：开始渐显（0%）
      fadeInner: 10,        // 射程 + 这么多：完全显示（100%）
      fade: 0.18,           // 强度插值时间常数（秒）。探测是 0.25s 一次的，不插值会看到台阶
    ...
    towerLight: {
      ...
      decay: 1.35,          // 衰减指数。2=物理正确但中心过曝/边缘断崖；1.2~1.5 才是"火柴"那种平滑扩散
      edgeLux: 0.16,        // 半径边缘处的照度目标。调亮整体就调它
      centerClampLux: 3.2,  // 中心照度上限：decay<2 时中心仍会很亮，超过这个值就把强度压回来，免得塔脚一片死白
      ...
      // 灯池不够用时，给够不上灯的塔铺一层地面辉光贴片：数量无上限、不参与光照计算，
      // 所以照不亮小兵，只补"这座塔也在发光"的观感。半径与真光源一致。
      glowDecal: true,
      glowOpacity: 0.3,     // 贴片中心不透明度（边缘按 smoothstep 衰减到 0）
      glowSoftness: 0.55,   // 0=硬边圆盘，1=整片糊开；控制中心→边缘的过渡宽度
    },
    // 真 HDR 输出（用户点名要）。
