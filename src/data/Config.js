...
export const CONFIG = {
  ...
  world: {
    ...
    //
    // 新增的加成类型（用户确认方向）：
    //   防御类（护甲/魔抗）—— armorFlat/magicResistFlat。
    //   法力获取加成（不是法力回复！）—— manaGainPct，见 statDocs.js 里这个字段
    //     的说明："法力获取加成"，同时影响被动回复与攻击/受击回蓝三条来源。
    //   移速/射程类在现有基础上加多一项 —— 白天原来只有 moveSpeedPct，这次补上
    //     attackRangeFlat（小兵多一点点主动接团/越塔支援的空间）；夜晚原来的
    //     attackRangeFlat 不变——塔本身不会移动，"移速"这个方向对塔没有意义，
    //     所以这一项只加在白天/小兵这边，不是机械地两边各加一份。
    //   原有的"+攻击力%"改成"+适应之力"（用户："原有的加攻击可以改成+适应之力
    //     感觉比较好"）——适应之力会按这个单位当前物理/法术谁更强自动转化成对应
    //     属性（见 AttributeCalculator 的 adaptiveForce 处理），比无脑给攻击力更
    //     贴合本轮刚做完的自适应伤害重做（远程/图腾/术士/蚀骨已经是魔法输出为主）。
    //   明确排除：生命回复（用户："不要做生命回复"）。
    // 下面这些是【满档（100%，即正午/极夜那一刻）】的数值，其它三档按天气同一套
    // 系数（25/50/75%）打折；具体强弱本轮只是"先给一版看手感"，不是平衡定案，
    // 需要的话后续可以跑 balance_matrix 或让用户直接试玩调整。
    dayNightBonus: {
      day: {
        moveSpeedPct: 10,       // 小兵：推得更快（原6，本轮调高凸显手感）
        attackRangeFlat: 12,    // 小兵：新增，射程小幅提升
        adaptiveForce: 12,      // 小兵：原 attackDamagePct:5 改为适应之力
        armorFlat: 5,           // 小兵：新增防御类
        magicResistFlat: 5,     // 小兵：新增防御类
        manaGainPct: 15,        // 小兵：新增法力获取加成
      },
      night: {
        attackRangeFlat: 20,    // 防御塔：覆盖更远（原15，本轮调高）
        adaptiveForce: 16,      // 防御塔：原 attackDamagePct:8 改为适应之力
        armorFlat: 6,           // 防御塔：新增防御类
        magicResistFlat: 6,     // 防御塔：新增防御类
        manaGainPct: 15,        // 防御塔：新增法力获取加成（装了主动技能武器的塔才用得上）
      },
