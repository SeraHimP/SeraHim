...
export const CONFIG = {
  ...
  ui: {
    // v51.27（Q1）：用户"增加特效……如果某单位在固定时间内要增加一定数额的值，
    // 就会在进度条高位出出现'拖尾特效'同款的一个显示（但是不要长得一样要做区分），
    // 就是告诉这个单位要回这么多血或者是法力。" —— 预告的是【接下来 windowSec 秒
    // 内按当前被动回复速率能回到的量】，用 regenPerSec×windowSec 换算成条上的一段
    // 宽度，画在真实血量/法力量的紧邻高位（拖尾画在低位/掉血侧，这个画在高位/
    // 回复侧，位置上天然区分；颜色再选一个跟拖尾的淡红完全不同的暖黄，双重区分）。
    // 只覆盖"持续被动回复"这一种来源（生命恢复/法力恢复），不含主动技能的瞬间
    // 治疗——那类没有一个稳定的"未来速率"可供预告，超出这条特效能表达的范围。
    barIncreasePreview: {
      enabled: true,
      windowSec: 1.0,                       // 预告未来多少秒内的回复量
      color: 'rgba(255, 205, 90, 0.65)',    // 暖黄，与拖尾的淡红（TRAIL_COLOR）区分
    },
