...
export const CONFIG = {
  gameRules: {
    ...
    supportUnits: {
      ...
      healer: {
        ...
        // 2026-09-19 修复：治疗兵原来没有专属移动分支，走的是给"追敌人打架"用的
        // 通用小兵AI——于是它会锚定在离自己最近的【敌方】单位旁边站桩（普攻力0，
        // 站在那什么也打不出去），治疗全靠"刚好蹭到"半径内的友军。用户原话：
        // "治疗兵要贴着受治疗的友方单位"。见 LaneMovementSystem._updateHealer。
        searchRange: 700,             // 移动决策用：多远内"看得见"需要照顾的友军（比治疗半径更远，
                                       // 这样能先看见远处的伤兵、主动走过去，而不是等对方自己凑近）
    ...
    // 2026-09-26 第二轮：用户定稿"据点产生的兵改为每方向2近战1远程"——
    // ranged 2→1（melee/炮兵节奏不变）。这也顺带是"思考如何改进滚雪球"的
    // 一部分：据点出的兵少了一个，领先方靠据点滚出来的兵力优势跟着降了一档，
    // 但不是用户提这条的主要理由（主要理由是水晶掉血公式，见 nexusDrainPerPointPerSec
    // 头注），这里只是记录客观上的连带效果。
    // 2026-09-26 第三轮：用户追加"据点处不再生成炮兵"——原来的"每两波+1炮兵"
    // 整条规则删除，据点出兵从此只有 2 近战 + 1 远程，不含任何炮兵。
  ...
  //
  // ==================== v51.18：用户重新定稿明确胜率目标——75% ====================
  // 用户原话："拥有龙魂的队伍强一些是正常的。拥有龙魂的队伍胜率要保证75%！"——
  // 收回 v51.4 那次放宽，改回一个具体数字：目标胜率 75%（不是区间，是这一个点，
  // 允许小样本（20局/档）噪声导致的 ±1~2 局摆动）。本轮用这份最新对照数据
  // （2026-09-22，用户本机16核实测，20局/档，单局不设时长上限）按"扣基线后的
  // 推进度差，缩放到能让胜率落到75%附近"重新估了一遍全部13条魂，只动每条魂
  // 已经验证过的"真正吃重"那个/那几个杠杆（沿用v51.8/v51.13/v51.15~17一路排查
  // 出来的结论，不重新瞎猜）：
  //   基线45%（不是魂，红方天生略占优，历史遗留问题，Task#74已排查过，
  //           这次不再动）
  //   fire 70%/+0.93、water 60%/+0.39、earth 65%/+0.33、thunder 60%/+0.58、
  //   dark 60%/+0.15、steel 60%/+0.27、blood 65%/+0.61、magma 60%/+0.25、
  //   astral 60%/+0.18、rift 55%/+0.02 —— 全部低于75%目标，按各自杠杆同比例加强。
  //   poison 40%/-0.54、frost 45%/-0.16 —— 差距最大的两条，加强幅度也最大。
  //   wind 90%/+1.3 —— 唯一超标的一条，同一根杠杆往下砍。
  // 这依然是粗估（单点数据倒推的比例），不是最终数值——下一轮 --sweep soul 还要
  // 再验一遍，尤其是 poison/frost/wind 这三条改动幅度最大的。
  dragonSouls: {
    ...
    stat: {
      ...
      // v51.18：90%/+1.3，14档里唯一超过75%目标的一条，常驻加持三项一起同比例砍
      // （×0.63，跟下面机制层 wind.perStackPct 8→5 同一个缩放倍率，两层一起动才
      // 不会砍了一层另一层还是超标——参照 v51.8 "只砍一项等于没砍"的教训）。
      wind:    { bonusAttackSpeedPct: 6, attackSpeedRatio: 0.04, evasionPct: 6 },
    ...
    //
    // 🔥 炎魂：攻击附带溅射。
    fire:    { pct: 22, radius: 75, cooldown: 0 },   // v51.3：30 → 18（×0.59，主伤害杠杆）；v51.18：70%/+0.93，低于75%目标，18→22（×1.22）
    // 🌊 潮魂：攻击后回复已损生命 + 治疗强度。回血**不无视**加固城防的节点封顶
    water:   { healMissingPct: 3.5, powerPct: 25, buffSec: 5, cooldown: 8 },   // v51.3：70% 压线，不动；v51.7：healMissingPct 5→2（×0.42——20局0胜，胜率数字骗人，实际偏强）；v51.18：60%/+0.39，低于75%目标，2→3.5（×1.75）
    // 🗿 山魂：减伤 + 格挡。
    ...
    earth:   { damageReduction: 9, damageBlock: 1 },   // v51.18：65%/+0.33，低于75%目标，damageReduction 6→9（×1.5，damageBlock 那条 v44 定稿"不要再往上调"不动）
    ...
    thunder: { perTargetPct: 22, targets: 6, range: 200, cooldown: 8 },   // v51.11：1→15（恢复 v51.3 原值，见上）；v51.18：60%/+0.58，低于75%目标，15→22（×1.47）
    ...
    wind:    { perStackPct: 5, maxStacks: 5, decaySec: 3 },   // v51.18：90%/+1.3，唯一超过75%目标的一条（14档里最强），perStackPct 8→5（×0.63）；常驻加持层（下面 stat.wind）一起同比例砍，见那边注释
    // 🌑 暗魂：命中削双抗，**全队共享层数**（友军攻击也叠）；"偷取"机制自身自带 2 倍
    // 效果（既削对方又补自己）。
    dark:    { flatPerStack: 1.5, pctPerStack: 0.5, maxFlat: 40, maxPct: 15, duration: 6, steal: true },   // v51.11：pctPerStack 0.08→0.5、maxFlat 10→30、maxPct 2→15（全部恢复到本session改动前的原值）；v51.13 实测落在参照带内（+1.09），不动；v51.18：新目标75%下实测60%/+0.15，偏弱，真正吃重的杠杆是flatPerStack/maxFlat（v51.8已验证），flatPerStack 1→1.5、maxFlat 30→40
    ...
    poison:  { pctPerStack: 0.06, duration: 4, vsBuildingPct: 25, maxStacks: 30 },   // v51.11：0.02→0.3（恢复到 v51.3 改动前的原值）；v51.13：0.3→0.08（本轮实测离谱超标，×0.27）；v51.16：0.08→0.03（再次实测仍偏强+2.23，×0.36）；v51.17：maxStacks 999→30（真正的病灶：命中会刷新共用计时器导致层数无限走高，不是 pctPerStack 数值问题，见上方说明）；v51.18：maxStacks 病灶修好之后本轮实测反而是14档里最弱的一条（40%/-0.54，扣基线后是负数），说明v51.17那次在病灶修好后没跟着把pctPerStack往回调，矫枉过正，0.03→0.06（×2）
    // ==================== v50：六条新魂的机制参数 ====================
    ...
    frost:   { stacksToFreeze: 5, freezeSec: 1.2, immuneSec: 15, stackDuration: 4,
               towerAtkSpeedPct: -4, towerDebuffSec: 4, towerMaxStacks: 8 },   // v51.18：新目标75%下实测45%/-0.16（扣基线后是负数），14档里第二弱，towerAtkSpeedPct -2→-4（×2，跟v51.6"给少一点"那次同一根杠杆，这次是反方向调）
    ...
    steel:   { everySec: 8, flat: 33, maxHPPct: 0.9, missingHPPct: 2, currentHPPct: 0,
               reflectPct: 30 },   // v51.11：flat 20→120、maxHPPct 0.5→3、missingHPPct 1→6、reflectPct 5→30（全部恢复原值）；v51.13：flat 120→35、maxHPPct 3→1、missingHPPct 6→2（本轮实测离谱超标，三项一起×0.3，reflectPct不动）；v51.16：35→15、1→0.4、2→0.9（再次实测仍偏强+1.87，三项一起×0.44）；v51.18：新目标75%下实测60%/+0.27，偏弱，护盾三项一起×1.5（15→22、0.4→0.6、0.9→1.3）；v51.19：2026-09-24 --sweep soul 实测 45%/扣基线+0.11——与基线（无魂）几乎无法区分，14 档里事实上的垫底，比 v51.18 自己那次测出来的 60%/+0.27 还差，护盾三项继续同比例×1.5（22→33、0.6→0.9、1.3→2），reflectPct 不动（v44 定稿的次要分支，一直没被验证是主要杠杆）
    ...
    blood:   { peakAtHPPct: 33, attackDamagePct: 17, bonusAttackSpeedPct: 14, lifeStealPct: 7 },   // v51.11：attackDamagePct 9→30、bonusAttackSpeedPct 8→25（恢复原值）；v51.13：30→13、25→11、10→5（本轮实测中等超标，三项一起×0.44）；v51.18：新目标75%下实测65%/+0.61，偏弱，三项一起×1.3（13→17、11→14、5→7）
    ...
    magma:   { duration: 4, radius: 70, tickDamagePct: 0.42, slowPct: 12, splashPct: 24 },   // v51.13：0.6→0.3、20→6（砍错了杠杆，本轮验证纹丝不动）；v51.15：tickDamagePct/slowPct 都恢复回 v51.11 原值，新增 splashPct=35（真正的杠杆，修好滚雪球 bug 之后半径内其他敌人只吃35%伤害）；v51.16：35→12（再次实测仍偏强+2.63，×0.34）；v51.17：tickDamagePct 0.6→0.2、slowPct 20→6（splashPct 再砍纹丝不动，改砍主目标自己这两项，×0.3）；v51.18：新目标75%下实测60%/+0.25，偏弱，三项一起×1.4（0.2→0.28、6→8、12→16）；v51.19：2026-09-24 --sweep soul 实测 50%/扣基线+0.09——比 v51.18 自己测出来的 60%/+0.25 还弱，14 档里事实上的垫底（数值上等同没拿），三项继续同比例×1.5（0.28→0.42、8→12、16→24）；如果下一轮还是纹丝不动，要怀疑是不是还有第四处结构性问题没找到（这条魂已经因为"砍错杠杆"翻过两次车，splashPct 那次和均摊那次），不能一直在这三个数字里反复猜
    // 🌌 星魂：命中后分裂两枚小弹打向最近的敌人。
    ...
    astral:  { splits: 2, damagePct: 12, radius: 260, onHitEffPct: 55 },   // v51.11：damagePct 2→40（恢复原值）；v51.13：40→10（本轮实测离谱超标，×0.25）；v51.18：新目标75%下实测60%/+0.18，偏弱，10→18（×1.8）；v51.19：2026-09-24 --sweep soul 实测 85%/扣基线+1.50——14 档里最离谱的一档（比第二名 wind 的 +1.07 还高出不少），说明 v51.18 那次 10→18 的×1.8 涨幅严重过头，两个端点（10→60%/+0.18太弱，18→85%/+1.50太强）之间响应明显不是线性的，这次不按线性插值硬算比例，保守回落到中间偏低的 12，下一轮数据出来再看这段区间的响应形状
    ...
    rift:    { perStack: 4, maxStacks: 5, duration: 6 },   // v51.11：perStack 1→4（恢复原值）；v51.13：4→2（本轮实测中等超标，×0.5）；v51.18：新目标75%下实测55%/+0.02（扣基线后基本为0，14档里离目标最远的之一），2→4（×2，恢复回v51.11原值）
  ...
  dragonPower: {
    ...
    earth:   { maxHPPct: 1.5, armor: 2.5, magicResist: 2.5 },   // v51.19：2026-09-24 --sweep power 实测 80%胜率/扣基线+1.25——目标带是"落在基线+0.3~+1.0"（力是过程奖励，弱于魂），这条从没调过，首次超标，三项一起×0.6（2.5→1.5、4→2.5、4→2.5）
    ...
    // v51.18：2026-09-22 --sweep power 实测 55%/-0.09（扣基线后基本为0，等于白拿），
    // 目标带是"推进度差落在基线+0.3~+1.0"（力是过程奖励，弱于魂），这条完全没到
    // 下限，armorPenPercent/magicPenPercent 一起×2。
    thunder: { armorPenPercent: 7, magicPenPercent: 7 },   // v51.19：2026-09-24 --sweep power 实测 80%胜率/扣基线+1.31——v51.18 那次 5→10 的×2 涨幅严重过头（5 那档测出来是"基本白拿"，10 这档变成 14 档里并列最强），响应同样不是线性的，回落到中间值 7 而不是退回 5，下一轮数据再看响应形状
    ...
    // v51.18：2026-09-22 --sweep power 实测 100%胜率/+2.79（扣基线），14档里唯一
    // 20局全胜的一档，目标带上限只有+1.0——是全部13条巨龙之力里唯一"力比魂还强"
    // 的一条（对照同一批数据里 astral 魂只有60%/+0.18，力反而比魂猛得多，完全
    // 违反"力是过程奖励，弱于魂"这条设计前提）。射程/弹速这类"命中判定"型属性
    // 对塔（钉死不动、永远索敌命中）的收益本来就比对会走位的单位更稳定，攻击
    // 频率没变的情况下射程每涨一截都是净赚的输出窗口，这次两项一起大砍
    // （attackRange 8→2、bulletSpeed 20→6，都是原来的约1/3.3~1/4），砍完之后
    // 如果还是偏强，下一步要怀疑是否有结构性问题（类似之前 magma 溅射滚雪球
    // 那种），不能一直在这两个数字里反复砍。
    astral:  { attackRange: 2, bulletSpeed: 6 },
    // v51.18：2026-09-22 --sweep power 实测 45%/-0.06（扣基线后基本为0，等于白拿，
    // 13条力里最弱的两条之一），damageReduction 1.5→3（×2）。
    rift:    { damageReduction: 2 },   // v51.19：2026-09-24 --sweep power 实测 85%胜率/扣基线+1.29——14 档里唯一 20 局胜过 17 局的一档，比 v51.18 自己那次 100%/+2.79 砍完之后本该收敛的效果又反弹回来了，同样是 v51.18 那次 1.5→3 的×2 涨幅过头，回落到中间值 2
  ...
  ui: {
    ...
    // 2026-09-19：牧灵塔（weapon_shepherd）不攻击，不该画"正在输出"的红线——
    // 用户原话："塔本身就不要再显示攻击红线了，而是显示绿线和幻兽相连"。
    // 复用 aimLine 同一套宽度/透明度，只是换一个颜色和语义（"拴绳"而不是"瞄准"）。
    petLeashLine: {
      color: '#3ce85c',
    },
    ...
    // ==================== 塔"被雪覆盖"效果（v55.3）====================
    // 用户要求"这个雪的效果能不能做在塔上"，同一套 applySnowTint 机制照搬到塔身
    // InstancedMesh（见 InstancedBodyLayer.js 的 BodyBucket）。塔数量少（≤44），
    // 没有像野区那样"上千实例"的开销顾虑，但仍然沿用节流刷新的模式，跟植被口径
    // 保持一致，避免"一个地方节流一个地方不节流"这种不必要的不对称。
    towerSnowFx: {
      maxBlend: 0.55,        // 塔身比树冠小、细节更少，封顶混合比例调低一点，避免整座塔糊成一片白
    ...
    // UnitMeshFactory.crystalMaterial 头注的完整排查记录。power 越大，"亮边"收得
    // 越窄（更贴近轮廓线）；strength 是叠加到自发光上的倍率，跟着
    // emissiveIntensity（含攻击蓄能）同步放大，不是独立的一份亮度。
    crystalRim: {
      power: 2.5,     // Fresnel 指数：越大边缘光带越窄越锐利
    ...
    groundTraceFx: {
      ...
      // ==================== v55.4 修复：野区雪盖在深色地面上"看着还是绿的" ====================
      // 用户报"野区依旧没有雪覆盖"：排查确认雪盖数据/纹理/材质全部正确（哪怕强制
      // 局部雪深=1，画面实测也一样）——真根因不是数据没生效，是【同一档不透明度
      // 在不同饱和度的地面色上观感天差地别】：野区地面色（forest 调色板的
      // jungleColor '#4c9a5b' 一类饱和绿）用 60% 白色去混，数学上算出来是
      // (183,215,189) 这种"浅薄荷绿"——依然读得出绿，不像"雪"；而路面色本来就浅
      // （'#c2a06a' 暖棕），同样 60% 白混出来是 (231,217,195) 这种奶白色，看着才
      // 像雪。同一个不透明度公式，套在两种饱和度差很多的地面色上，观感完全不对等，
      // 不是把哪个数值改大改小的问题，是野区需要更高的不透明度才能达到路面那种
      // "看得出是雪"的观感——这就是这个新增系数的用途：只放大野区（zoneMix=1）
      // 那一路的不透明度，路面（zoneMix=0）沿用原来的 snowCoverAlpha，画面不变。
      // 1.5 是按"目标混合比例覆盖住野区最常见的普通森林绿(jungleColor)"反推的——
      // 0.6×1.5=0.9，比路面的0.6明显更白，足够盖过饱和绿；没有细分林缘/深林三档
      // （zoneMix 目前只有路/野区二分，见 _ensureSnowGrid 头注），深林的更深绿色
      // 理论上还需要更高的系数才能完全"看不出底色"，这里先解决"完全看不出雪"这个
      // 主诉求，三档精细化留给以后有需要再做。
      snowCoverJungleAlphaBoost: 1.5,
    ...
    // ==================== v55.9：河道水面随气温降温变色 ====================
    // 用户看实机截图反馈："不好看这个水，和环境是割裂的"——WaterLayer.js 原来
        // 只有一个固定色（0x35707c 深青绿），不随天气变化；寒潮/大雪把全图压成白/冷
    // 色调后，这块固定饱和色的水面在视觉上完全不参与环境变化，显得像贴上去的一
    // 块色块。用户选定方向："降温变色"——保持是液态水（不换成结冰材质），只是
    // 天冷时颜色往冷灰蓝调，跟周围雪景融合；温度轴复用 WeatherSystem.getTemperature()
    // 已有的 -1(最冷)~+1(最热) 读数，跟 tempWarmColor/tempCoolColor 那组走的是
    // 同一根轴，但这里只关心"冷"这一侧（暖天水色不变，没有"水变热"这种说法）。
    water: {
      color: '#35707c',       // 常温水面基础色（与改动前 WaterLayer.js 里的 0x35707c 一致）
      coldColor: '#8199a3',   // 严寒时混向的冷灰蓝——比常温色浅、更接近雪景色调，但仍读得出"是水"
      opacity: 0.55,
    },
  ...
  groundTrace: {
    puddle: {
      ...
      // ==================== v55.7：减速幅度按水洼"大小"再叠一层 ====================
      // 用户原话："水洼越大移速降得也多"——改之前减速只乘 strength（水洼从"刚冒头"
      // 长到"完全成型"的时间进度，0~1），跟水洼实际有多大（合并了多少子圆、覆盖
      // 多大面积）完全没关系：一个孤立的小水洼放久了 strength 也能到 1，跟一片
      // 连成一大滩的水洼强度相同，减速却一样多，不符合"越大越滑"的直觉。
      // 用水洼的外接半径 p.r（合并越多子圆，这个半径越大，见 _boundingRadius）
      // 归一化出一个 0~1 的尺寸系数，乘进最终减速里。
      // ==================== v55.9 修复：260 这个参照半径凭空估的，脱离实际 ====================
      // 最初按"合并两三片单个水洼"口算估了 260，但没有实测——用户报后来复查发现：
      // 跑一局 20 分钟持续大雨的模拟，_makeSubOffsets 每个水洼一生成就有 3~6 个
      // 子圆，外接半径天生就在 90~150 一带，即使持续合并/生长 20 分钟，观测到的
      // 最大值也就 245（且是极端个例，大多数聚在 120~160）——260 这个参照值全场
      // 几乎摸不到，所有水洼的尺寸系数因此几乎全部卡在 minSizeSlowFactor 的下限，
      // 等于"按大小缩放"这条今天刚加的机制在实际对局里从来没有真正生效过，跟野区
      // 雪盖不可见是同一类"数值凭空拍、没有用真实分布验证"的错误。改成 180——
      // 用同一份 20 分钟模拟的半径分布重新标定，180 能让观测到的中位数水洼落在
      // 0.7~0.8 这个有明显区分度的区间，只有个别极端连片水洼才会顶到封顶 1.0，
      // 小水洼也不会全部一样卡在地板值。
      sizeSlowRefRadius: 180,  // 水洼外接半径达到这个值，尺寸系数封顶=1
      minSizeSlowFactor: 0.5,  // 尺寸系数下限（刚生成的孤立小水洼不会低于这个比例）
    ...
    snowCover: {
      ...
      // 2026-09-19 第一次修复：growPerSec/decayPerSec 原来是 0.02/0.03，把 snowGlobalTarget
      // 的推进从"指数逼近"的时间常数拉长到 33s/20s，以为"时间常数越长=看起来越慢"
      // 就够了——用户第三次反馈同一个症状，才发现问题出在**曲线形状**而不是速度：
      // 指数逼近天生是"起步最快、越接近终点涨得越慢"，哪怕总时长拉到几分钟，
      // 开局那 20 秒（用 τ=33s 算）也已经吃掉了将近一半的总变化量——人眼看到的
      // 仍然是"唰"一下奔向快半白，跟总时长有没有拉长无关。
      // 第二次修复（真正对症）：GroundTraceSystem._updateSnowCover 把推进公式从
      // `x += (target-x)*rate`（指数）换成 `x += rate*dt`（匀速线性），全程变化率
      // 恒定，不再有"前段特别快"的尖峰。growPerSec/decayPerSec 的含义也跟着变了：
      // 不再是指数系数，是直接的"每秒增减多少"，1/growPerSec 就是 0→1 所需的
      // 精确秒数。下面标定为匀速120秒铺满、匀速80秒退净（decay 比 grow 快，
      // "融雪比积雪快"的既有比例关系不变）。
      growPerSec: 1 / 120,    // 全局目标雪深匀速增长速率（每秒增量），120秒铺满全图
      decayPerSec: 1 / 80,    // 雪停后全局目标雪深匀速消退速率（每秒减量），80秒退净
      maxDepth: 1,            // 雪深上限
      regrowPerSec: 0.05,     // 每格局部雪深追向全局目标值的速率（含小径回涨）
      // ==================== 2026-09-20 第三次修复：均匀铺满本身就是"突变"====================
      // 用户第四次反馈同一个症状（前两次分别修了"时间常数太短"和"指数曲线形状"，
      // 这次症状依旧）。重新审查后发现：即使 snowGlobalTarget 的推进已经是匀速线性、
      // 每一格追向它的速率也是同一条公式，但**全图每一格用的是完全相同的速率**——
      // 于是全图所有格子永远同步同速逼近同一个目标，人眼看到的是"整块地面在同一时刻
      // 一起变白"，这本身就是"突变感"的来源，跟曲线形状/时间常数都无关：真实下雪
      // 是有的地方先积、有的地方后积，斑驳地铺开，不是整张地图同时刷白。
      // 修法：给每一格再叠一个【空间噪声】速率倍率（cellRateNoiseAmp 控制幅度，
      // 复用 mapValidate.js laneWidthNoise 同款"两个不同频率正弦波叠加"做法——
      // 那边注释写得很清楚："哈希是给散点装饰用的（每个点互不相关），这里要的是
      // 连续、平滑的起伏，用哈希会变成锯齿状抖动而不是自然的起伏"，这里同理）。
      // 各格倍率固定不变（只在网格首次建立时算一次），有的格子天生快、有的天生慢，
      // 最终都收敛到同一个目标，但"谁先谁后"从空间上错开，看起来是自然蔓延，
      // 不是屏幕级联的同步渐变。
      cellRateNoiseAmp: 0.65, // 每格速率倍率的浮动幅度：倍率 = clamp(1 + noise×amp, floor, 1+amp)
      cellRateNoiseFloor: 0.25, // 倍率下限（不能低到某些格子几乎永远追不上目标）
      erodeRadius: 70,        // 单位经过时，局部雪深被踩低的影响半径（世界单位）
      // ==================== v55.9 修复：erodePerSec 1.2 → 0.1 ====================
      // 用户实测："积雪的移速降低就是-6.6恒定的，即使小兵走的是前面小兵踩过的地方
      // 还是小兵新走的地方"——排查（真实速度78、erodeRadius=70 的移动小兵模拟）
      // 发现：旧值 1.2 意味着满雪深(1)到踩踏下限(pathFloor×target=0.3)只需要
      // 约 (1-0.3)/1.2≈0.58 秒——比小兵穿过 erodeRadius 覆盖范围所需时间
      // （2×70/78≈1.8秒）短得多，等于【小兵还没走出自己脚下这一圈影响半径，
      // 那圈地就已经被它自己踩到下限了】。结果是任何小兵只要正常行走（不是
      // 停在原地），落脚点几乎从第一秒起就恒定卡在 pathFloor 对应的减速值，
      // "雪越厚移速降得越多"这个效果实际上只在极短的瞬间存在，玩家根本感觉
      // 不到，这正是"恒定-6.6%"的真根因——不是数值没有按深度算，是深度本身
      // 被自己的脚步瞬间抹平了。
      // 改成 0.1：同样公式下满深→下限需要约7秒，实测单次经过全新雪地时深度
      // 基本维持在0.9附近（几乎不受自己这一次经过影响），但持续多个兵线波次
      // 反复碾压同一路段（一局比赛的量级）仍然会累积踩到接近下限，"小径"这个
      // 视觉/手感设计意图保留，只是不再在单次经过内就被自己抹平。
      erodePerSec: 0.1,
  ...
  stylizedPalettes: {
    ...
    // ==================== 统治战场·水晶之痕专用：黄沙风格 ====================
    // 用户定稿："为黄沙风格得地图"——参照 docs/TEXTURE-PROMPTS.md 里已经写好的
    // "沙漠峡谷"分区参照（橙棕色沙地+赭红岩壁+浅黄褐建筑+极少植被），这里只是把
    // 那份文字描述落成调色板数值，跟 frost/forest 一样走"地形整体一个色相"这套
    // 已验证的风格化路线，不新起一套配色逻辑。本图不需要"走廊 vs 野区"二分
    // （环形赛道本身就是唯一可走区域，没有森林梯度那种空间层次），沿用 frost
    // 同款 vegetationMode:'none'——沙漠据点稀疏植被这类装饰留到核心机制跑通、
    // 用户对着实机截图定稿细节之后再加（CLAUDE.md 第 4 条：视觉细节先对齐再做）。
    desert: {
      groundColor: '#3a2410',     // 环外虚空：暗赭褐，收着不抢戏（同 frost/forest 的 groundColor 用法）
      corridorColor: '#c9915a',   // 环形赛道本体：暖沙黄，TEXTURE-PROMPTS.md"沙漠峡谷"ground条目同色调
      treeTrunkColor: '#7a5a3a',  // 本图暂不生成植被，保留字段仅为结构完整（同 frost 的处理）
      treeCrownColorA: '#8a9a52',
      treeCrownColorB: '#a3b568',
      rockColor: '#9c5a42',       // 崖壁/岩石：赭红色，对应"沙漠峡谷"cliff 条目的赭红岩壁
      wallCapColor: '#d9b878',    // 压顶石：浅黄褐建筑色，与暖沙地形拉开一档明度但同一色相家族
      outlineOnByDefault: false,
      vegetationMode: 'none',
    },
    // ==================== v51.18：魔幻森林（扭曲丛林专用）====================
    // 用户与我先对齐过一版文字方案再动手（第四条铁律要求）："紫雾+扭曲发光树木
    // （推荐）：地面/雾效偏紫（紫罗兰~品红渐变），树木造型扭曲缠绕（不是SR那种
    // 规整松树），树冠/藤蔓带低强度自发光（紫/青），地表点缀发光孢子或菌类，
    // 整体阴森奇幻感。然后参考LOL中扭曲丛林的设计感"——用户确认这份方向。
    // 复用 forest palette 同一套地形结构（三级森林梯度 forestZoneAt + navgrid + 野区
    // 装饰层），这里只换主题元素：配色整体偏紫/品红，树用新增的 treeShape:'twisted'
    // 扭曲树干（见 VegetationLayer.stylizedTreeGeo），树冠色刻意选高饱和高明度的
    // 紫/青——VegetationLayer 目前用 MeshLambertMaterial（不支持真正的 emissive
    // 材质，改这个要动全部风格化地图共用的材质管线，风险面太大，这次先不碰），
    // 用"亮到发光"的颜色在暗紫背景上读出自发光的错觉，是个成立的取巧办法，
    // 不是敷衍。地表发光孢子/菌类这条本轮先不做——那是全新的散布装饰类型，
    ...
    magicForest: {
      ...
      treeTrunkColor: '#2a1830',
      treeCrownColorA: '#a855c9',
      treeCrownColorB: '#5ec9d6',
      treeCrownDeepA: '#7a3a94',
      treeCrownDeepB: '#3d9aa8',
      rockColor: '#4a3552',
      wallCapColor: '#c85ee0',
      outlineOnByDefault: false,
      vegetationMode: 'jungle',
  ...
  tuning: {
    ...
    // ==================== 2026-09-21：热寂终局（目前仅召唤师峡谷）====================
    // 起因：用户跑龙魂平衡（tools/run_balance_soul.mjs 默认不传 --minutes，子进程
    // balance_matrix.mjs 因此按"不设上限"跑到真分出胜负为止）挂了超过24小时依旧卡住——
    // 排查确认是设计内已知风险：两边打成真正的对称僵局时，主循环没有任何退出条件，
    // 会真的无限跑下去（tools/balance_matrix.mjs 头注早就写了这个后果）。
    // 用户定稿的解法不是给测试工具打补丁（那治标不治本，只覆盖跑批场景），而是在
    // **游戏本身**里加一条终局机制：拖得够久，游戏自己会把自己终结掉，真人对局和
    // 批量模拟都受益，也让 balance_matrix.mjs"默认不设时长上限"这个设计从此
    // 真正安全（不再需要额外的安全阀）。
    //
    // 机制（用户定稿，逐条确认过）：
    //   ① 到点即触发，不需要"过载"预警阶段——「过载」只是这个状态的名字，不是独立的
    //      更早阶段。
    //   ② 塔/水晶枢纽/召唤水晶：每秒损失【自身】当前最大生命值的 towerDrainPctPerSec%，
    //      走既有的"负生命恢复=字面扣血"通道（与天气负恢复同一套语义：不吃恢复加成，
    //      可致死，结构保护/无敌照常免疫）——即被保护的后排塔不会立刻掉血，得等
    //      前置层级先被熔穿曝光，是有意的（用户确认："被保护的后排塔不会立刻掉血"）。
    //   ③ 所有非塔单位（含中立单位，如巨龙）：攻速/伤害增幅/移速三项各 +100%。
    //   ④ 目前只有 summoners_rift_v1 声明了这条光环（用户："目前仅召唤师峡谷，
    //      后续再加其他地图"）——嚎哭深渊/扭曲丛林等其他地图的对局理论上仍可能
    //      真死锁，还没有这条终局保底，后续要加时照抄这份 CONFIG + 地图声明即可。
    heatDeath: {
      triggerAtMin: 70,          // 第几分钟触发（用户："时间改为70分钟"，原提案75分钟）
      towerDrainPctPerSec: 1,    // 塔/水晶枢纽/召唤水晶：每秒损失自身当前最大生命值的百分比
      unitAtkSpeedBonusPct: 100, // 非塔单位（含中立）：攻速加成
      unitDmgAmpBonusPct: 100,   // 非塔单位（含中立）：伤害增幅
      unitMoveSpeedBonusPct: 100,// 非塔单位（含中立）：移速加成
    },
    // 召唤师峡谷常驻光环（与热寂独立，从第0分钟就生效，不是热寂触发之后才有）：
    // 全体单位每分钟 +X% 移速，无上限（用户定稿："无上限"——与扭曲丛林攻速光环那条
    // 15% 封顶的先例不同，这条是故意不封顶的）。
    summonersRiftMoveSpeedPctPerMin: 0.5,

    // ==================== 汇流战场（超大型多战线地图）专用终局/节奏数值 ====================
    // 用户要求这张图"超大规模交战、多个战线可以重叠……最好交战时长在180分钟"。
    // 热寂机制本身（drain/攻速/伤害增幅/移速加成三项数值）复用上面 heatDeath 那组
    // 共享常量——不是另开一套倍率，只是触发时间点不同：不能直接套召唤师峡谷的
    // triggerAtMin(70)，那是按三路小图调的，会把这张明显更大、要打更久的图腰斩。
    // v1(165分钟)交付后跑了一批小样本实测（--runs 3 --minutes 90），3局全部在
    // 封顶前分出胜负，均时长仅60.83分钟——热寂根本没被触发过，塔的血量本身
    // 才是真正的瓶颈。塔血量已经上调（见 confluence.js tierStats 头注的"实测
    // 校准"），这里把触发时间也从165推迟到220：目标是让它继续只当"极端拖到
    // 天荒地老才生效的保险丝"，而不是常规对局会摸到的边界——180分钟是均值
    // 目标，220分钟留了充分余量给正常波动的长局。同样如实说明：调整后还没有
    // 再跑一轮完整批次验证。
    confluenceHeatDeathTriggerAtMin: 220,
    // 常驻移速光环速率比召唤师峡谷（0.5%/分钟）略低——地图物理尺寸更大，
    // 若沿用同一涨速会让后期移速涨得比召唤师峡谷更夸张，不成比例。
    confluenceMoveSpeedPctPerMin: 0.35,
  },
  ...
  // ==================== 统治战场·水晶之痕：核心占领机制 ====================
  // 见 docs/DOMINION-CRYSTAL-SCAR-DESIGN.md——用户拿这份设计问过 ChatGPT，
  // 逐条核实/校正后采纳（该文档第 5 节记了完整推理，这里只落数值）。
  // 全部数值都是**起草值**，等原型跑起来用 tools/balance_matrix.mjs 类似的
  // 批量对局工具校准（docs/DEVELOPMENT.md §9："调平衡不拍脑袋，先量分布"）。
  dominion: {
    // 占领进度：单一有符号值，[-100,100]。正=蓝方占优，负=红方占优，
    // 但"完全占领"（触发出兵/开火/计入水晶掉血）只在 ±100 这两个极值——
    // 中间的任何值都只是"正在拉扯"，两边都能继续往任意方向推。
    // 一旦某方占满 100，归属**粘住**该方：此后只有【对方】的攻击能把它从
    // 100 往 0 推，己方自己打不动它（canTarget 天然禁止同阵营互相攻击，
    // 这条不需要额外代码保证）；推到 0 才会变回中立，从 0 重新开始被两边争夺。
    captureFull: 100,
    // 每次命中据点，攻击方按自己的兵种类型贡献这么多"占领压力"（正负号由
    ...
    // 2026-09-26 第四轮：用户定稿"中立据点会正常攻击"，这三个百分比字段当时
    // 改成不分中立/被占领、一律套用（见 DominionSystem.initMap() 里据点实体的
    // baseStats）——归属只决定它认哪一方为敌，不再决定"打不打"。
    // 2026-09-26 第四轮·补充：用户追加定稿"据点在中立状态下攻击力低，在某方
    // 占领之后攻击力提高"——上面那条"一律套用"改窄成只覆盖射程/攻速这两项，
    // 攻击力单独拆出 pointNeutralDamagePct（中立时用，见下面）跟 pointDamagePct
    // （占领后用，数值不变，仍是这两轮调出来的 30%）区分开，见
    // DominionSystem.initMap()/_setOwner() 按 captureOwner 现在切换这一个字段。
    // 15% 是起草值（用户只给了方向"低"，没给具体数字），大致取占领后 30% 的
    // 一半，待 balance_matrix/实机校准。
    // 2026-09-26 第一轮：用户反馈"据点的攻击力大幅度减弱，攻速略微提升"——
    // 35% → 12%，新增 pointAttackSpeedPct=110%（+10%，"略微"）。
    // 2026-09-26 第二轮：用户实机测过之后改口"据点的攻击力太低了，需要加强，
    // 并且攻速改为1.5"——12% 确实砍过头了，改成 30%（用户没给具体数字，起草值，
    // 比原始 35% 略低，比上一轮的 12% 明显加强，待 balance_matrix 校准）；
    // 攻速这次给了具体数字"1.5"，按已有的 pointAttackSpeedPct（相对普通塔基础
    // 攻速的百分比）口径解读为 150%（不是绝对的每秒 1.5 次，那样脱离了这个
    // 字段原有的"相对值"语义，且与塔本身攻速无关系数不可比）。
    // 2026-09-26 第三轮：用户实机测过第二轮的 30%/15% 之后仍反馈"据点的攻击力
    // 还是太低了，中立状态和占领状态的攻击都提高"——这次没有给具体数字，仍是
    // 起草值，待 balance_matrix/实机校准。之前第二轮的注释写"比原始35%略低"是
    // 因为当时没想过要超过原型草案的基准，但用户这轮明确说"还是太低"，说明
    // 35% 这个原始基准本身就不是硬性上限，这次占领后攻击力 30%→50%，直接超过
    // 原始基准；中立时攻击力保持"约占领后一半"这个第四轮补充定的比例关系，
    // 15%→25%（50 的一半）。
    pointNeutralDamagePct: 25, // 中立时：普通塔攻击力的 25%（第三轮：15%→25%，约占领后一半）
    pointDamagePct: 50,        // 占领后：普通塔攻击力的 50%（第三轮：30%→50%，超过原始35%基准）
    // 2026-09-26 第五轮：82 → 130（用户定稿"增大每个据点的默认攻击距离"，
    // 没给具体数字，起草值，待 balance_matrix/实机校准）——配合同一轮"驻守
    // 小兵按据点攻击范围判定"的新机制（defenderSlowPct），据点自己的攻击
    ...
    // 2026-09-26 第二轮：用户反馈"目前某一方滚雪球太严重了，思考如何改进"——
    // 分析下来，掉血速率跟据点数差【线性】走是这套系统里唯一一个随差距扩大
    // 而扩大、还没有任何上限缓冲的杠杆（出兵编排本身不随差距变化，占领速度
    // 也跟差距无关）：一旦某方把 5 个据点全占了（这张图总共就 5 个据点），
    // 掉血速率是"领先 1 个据点"时的整整 5 倍——差距每多 1，就【等比】再加一份
    // 惩罚，越滚越快，正是"滚雪球"字面意思的机制来源。
    // 改成按 sqrt(据点数差) 走（见 DominionSystem._tickNexusDrain）：差距还是
    // 越大惩罚越重（保留"落后要更疼"这个核心设计意图，不是躺平不惩罚），但
    // 增速变缓——5:0 时惩罚只是 1:0 时的 √5≈2.24 倍，不再是 5 倍，前面每多
    // 拉开 1 个据点差带来的惩罚增量也逐级变小（边际递减），缓解"一旦形成优势
    // 就一路碾压到底"的单向雪球感。这是起草的缓解方案（用户只要求"思考如何
    // 改进"，没给具体公式），待 balance_matrix 实测校准，如果还是太猛/太缓，
    // 数值本身（这个字段）和公式形状（_tickNexusDrain 里的 sqrt）都可以再调。
    //
    // 2026-09-26 第四轮：用 tools/balance_dominion.mjs 跑过一局完整镜像对局
    // （双方数值/AI完全对称）后发现，真正的滚雪球根因不是"掉血太快"，是
    // "水晶枢纽没有任何回血（healthRegen 恒为0），掉的血永远追不回来"——
    // 实测里落后方后来靠追赶炮兵机制两次把占点数打平/反超，但因为之前落后
    // 期间欠的血债不会自愈，最终还是被翻旧账打死。这条根因本轮定稿暂不修
    // （用户："除了②之外的目前都不做，不过可以加到方案里以后再说"），先只用
    // 这个字段本身做一次缓冲：1 → 0.8（用 --drain-mult 扫过 0.5~4 的曲线，
    // 0.8 对应单局≈16分钟，比默认的1（≈14分钟）多给落后方一点点回旋时间，
    // 又不会像 0.5~0.7（21~31分钟）那样把节奏拖得太长）。这仍然是起草值，
    // 水晶枢纽 maxHP 用户明确要求"还是500"，本次没有跟着动。
    // 待办（已加进方案，本次不做）：
    //   ① 给水晶枢纽加"己方当前占点数不落后时才生效"的 healthRegen，跟这条
    //      掉血公式对称——这是根因级修复，比继续调这个系数更有效；
    //   ② 据点数差为1时也按 sqrt(1)=1 全速掉血，可以考虑加个死区（差距≥2
    //      才开始掉血）。
    nexusDrainPerPointPerSec: 0.8,
    // ==================== 2026-09-26 新增：据点争夺 + 脱战恢复 ====================
    // 用户定稿："两方不能同时占领据点，如果出现了，据点进入不可被占领状态
    // （不可被双方选中，迫使两方开始交战），直至只剩一方占领该据点。"
    // 判定：分别记录蓝/红两方【最近一次】在场（有小兵把这个据点设为目标）的
    // 时刻，两个时刻都落在 contestWindowSec 秒以内 → 判定为"双方都在场，正在
    // 争夺"，见 DominionSystem._tickCapture()。窗口不能太短（几乎每一帧扫描都
    // 会触发一次"刚刚又不在争夺了"的抖动）也不能太长（明明打完了、赢的一方
    // 却半天抢不回点），2 秒落在"一轮交战节奏"的量级上。
    contestWindowSec: 2,
    // ==================== 2026-09-26 第三轮：占领与攻击解耦 ====================
    // 用户定稿："小兵占领和攻击的实现是完全不同的……小兵占领据点时应该是固定
    // 每秒一次"——占领改成据点自己按这个固定节奏扫描"谁在场"，跟攻击者本身
    // 的攻速/攻击冷却完全没有关系（见 DominionSystem._tickCapture() 头注）。
    captureTickSec: 1,
    // 用户追加定稿："据点是有占领速度上限的，每秒最多10%最大占领速度"——不管
    // 同时有多少个/多强的小兵在占，一次 tick 的净变化量都夹在
    ...
    // ==================== 2026-09-26 用户追加两条终局/节奏机制 ====================
    // ①"若某阵营水晶枢纽生命值低于75，则该阵营每2次出兵额外生成一个超级兵"——
    // 用户给的两个数字都是具体值（75 是绝对生命值，不是百分比；用户原话没带
    ...
    // 口径（这里是用户明确给的【固定量】，不随水晶枢纽自身血量伸缩），所以没有
    // 复用 heatDeath 的字段，单独起了两个专属键。命名比照 confluenceHeatDeathTriggerAtMin
    // 这个既有先例（每张图独立配置触发时间点，不共用同一个全局阈值）。
    // 见 DominionSystem._tickNexusDrain() 里的落地：两座水晶枢纽无条件各扣一份，
