 ...
 *
 * ==================== 现有三张地图怎么办 ====================
 * 用户拍板：现有三张地图（召唤师峡谷/扭曲丛林/嚎哭深渊）**不迁移**——它们的
 * 源码继续保持现在这种"一个对象揉在一起"的写法，不拆分成独立的地形/配置文件
 * （不牵连它们身上大量的既有测试/平衡数据）。但用户明确想拿现有地图的**地形**
 * 当模板去配新打法（"塔位/兵线可能都不同"隐含的前提就是"地形能复用"），
 * 所以提供 `splitMapIntoTerrainAndConfig()`：从一张完整地图（不论是内置的还是
 * 自制的）现算出它的地形部分，供编辑器"以这张图的地形为模板新建配置"时用——
 * 这是只读提取，不改地图源码本身一个字。
 */
