...
const SR_CONFIG = {
  ...
  lanes: [
    {
      id: 'top',
      // 2026-09-04：跑 mapEditorCore.js 的 alignLaneToCorridor()（垂直切线双向探墙找
      // 走廊中点，见其头注）核实过——直线段贴边界的几个路点确实一致偏离走廊中心
      // 1~4 格，已按该算法输出吸附回中线；转角/枢纽端开阔地的点算法本身跳过未动。
      // v51.21：用户反馈"下路/上路兵线偏向路边"，重新跑了一遍同一个
      // alignLaneToCorridor（同一份代码、同一张 navgrid，没有改地图地形/走廊
      // 形状本身，只是把小兵实际走的这条【看不见的】路径重新贴回当前走廊的
      // 中线）——第304,821点偏了约7px，已吸附回297,820；其余点仍在中线上，不动。
      waypoints: [
        { x: 305, y: 3226 },
        { x: 262, y: 1120 },
        { x: 297, y: 820 },
    ...
    {
      id: 'bot',
      // 2026-09-04：同上（见 top 路点前的注释）——两个点吸附回走廊中线，其余点
      // 算法核实已经在中线附近，原样保留。
      // v51.21：同上一条注释——重新跑 alignLaneToCorridor 发现下路这次偏得更明显
      // （2442,3252 / 2742,3252 / 3262,2732 / 3262,2432 这四个点偏了约50px，
      // 明显靠向走廊一侧），已按算法输出吸附回中线；没有改地形/navgrid，只是把
      // 这条看不见的行军路径重新对齐到现有道路的正中间。移一个点会改变相邻点用来
      // 定位"垂直方向"的切线，所以迭代跑了几轮直到基本收敛（个别点在半格
      // 内来回摆动，取其中点）。
      waypoints: [
        { x: 305, y: 3226 },
        { x: 2441, y: 3307 },
