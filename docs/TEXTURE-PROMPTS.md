...
引擎默认**不去请求**这些分区图（`CONFIG.ui.zoneTextures.probe = false`），
一律用程序化占位图。理由是：文件不存在时每次启动会打十来条 404，
真正的报错就淹在噪声里了 —— 本仓库为此吃过亏，专门留了这条规矩。

所以流程是：

1. 按上面的命名把 PNG 放进 `assets/textures/<主题>/`
2. 把 `CONFIG.ui.zoneTextures.probe` 改成 `true`（编辑器里可改，不用动源码）
3. 刷新页面

缺哪一张就仍然用那一张的占位图，不会报错、不会白屏，所以可以一张一张地补。
