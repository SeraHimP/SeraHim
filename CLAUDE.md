...
前者是 `CONFIG.*Overrides`（见上一节）；后者住在这里：

| 类型 | 存在哪 | 是什么 | 难点 |
|---|---|---|---|
| 状态 | `CONFIG.customEffects[id]` | 纯数据蓝图 | 无。`EffectRegistry.apply` 本来就吃这个形状 |
...
载入靠 `customContent.syncAll()`（`main.js` 启动时 + 每次导入存档后各调一次，**幂等**）。
状态/兵种是数据，查一下就行；**技能不是**——引擎读的是 SkillLibrary 里的函数形状，
所以自制技能必须编译并注册，这一步不做就是"存档里有、用起来没有"。

### 为什么技能用声明式而不是让用户写代码

`eval` / `new Function` 这条路**不要走**：存档会变成可执行代码（导入别人的配置
等于运行别人的程序）、闭包无法序列化（存了盘读回来就没了）、报错抛在引擎深处。
...
（`ACTIONS[x].fields` 就是表单字段）——不要在 UI 里再抄一份清单。
...
1. **校验在保存时做完，不要留到运行时。** 技能每帧跑几百次，运行时校验又慢又会把
   同一条错误刷满日志；而用户需要的是"点保存那一刻"就知道哪写错了。
   报错信息要指出**第几条规则、哪个字段、合法取值是什么**——`invalid spec` 等于没提示。
2. **删除要连带清理。** 删状态前查有没有技能在用它（否则那个技能的 `applyEffect`
   静默空转，用户以为技能坏了）；删技能要从 `SkillLibrary._registry` 摘掉（否则
   已装备的实体继续跑一个配置里已不存在的技能，导出看不到、行为还在）；删兵种要清掉
   出兵编排里引用它的规则（否则 `buildWaveOrder` 拿到不存在的类型，创建时静默返回
   null，表现是"这一波莫名少了几个兵"）。
...
- **自制兵种的外观**：`MINION_STYLE` 里没有它。各处**统一走 `minionStyle(type)`**，
  ...
  用户做出来的每个兵都会变成问号（`ram` 那条注释记的就是同一个坑的第一次）。
  几何体会自动回退到通用步兵，不会崩。
- **兵种类型列表不要写死**。编辑器的 `_TPL_MINION_TYPES` 已改为 `allMinionTypes()`。
  写死的话自制兵在界面上根本看不见 = 等于没做出来。
- **`onFrame` 的周期计时**：`dt` 是 `1/30` 这种二进制表示不精确的数，累加 30 次得到
  `0.9999999999999999`。朴素的 `t < every` 会**每个周期都少触发一次**（"每秒一次"
  变成"每 1.03 秒一次"且持续漂移）；触发后也不能把计时器清 0，要减掉一个周期
  **保留余量**。两个都在 `behaviorVM` 里踩过。
