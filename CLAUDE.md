# SeraHim —— 给 AI 协作者的入口页

浏览器塔防（召唤师峡谷复刻），纯 JS + Three.js r169（2.5D），**无构建步骤**、
无 npm 依赖（Three.js 与后处理都已 vendored 到 `vendor/`）。直接开 `index.html` 就跑。

**完整开发设计规范在 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)** ——
架构、数据流、编辑器、自制内容、地图、渲染层、测试规范、调平衡、界面规范，
以及一份"症状 → 真实根因"的事故档案。**动手前请完整读一遍那份文件。**

本页只留**四条硬约束**：违反其中任何一条，改动都算失败。别的可以后看，这四条得先看。

> **v43 起，原第 1 条「行尾必须逐文件保持原样」已作废。**
> 全仓库已统一为 **LF、无 BOM**，`.gitattributes` 改成 `* text=auto eol=lf`。
> 直接用普通编辑器/脚本改文件即可，不用再探测和保留行尾。
> 作废的理由与那次归一提交的做法写在 `.gitattributes` 的注释里。
> 唯一仍然成立的老教训：**用脚本改文件时先算出新字符串再打开写句柄**
> ——`open(p,'w')` 会立刻截断文件，曾经把 `ModeDialog.js` 清成 0 字节。

---

## 1. 交付前 `npm test` 必须全绿

```bash
node tests/run_all.mjs      # 约 5~8 分钟，41 套
```

**基线是 0 失败。** 不要用"失败集合没变"当通过标准（本项目曾长期带着 6 个失败用例，
后来查明 5 个是断言自己写错了，不是产品 bug）。

新增行为**必须**配一套 `tests/sim_*.mjs` 并注册进 `tests/run_all.mjs` 的 `suites` 数组，
否则等于没做——没有断言守着的行为，下一个人改两行就没了。断言要钉**行为形状**，
不要钉具体数字（见 `docs/DEVELOPMENT.md` §8.2）。

## 2. 一切数值都必须软编码

用户的原话：「所有的都不要硬编码，都应该是可编辑的软编码」。
新数值一律进 `src/data/Config.js`，并在编辑器里给出可改入口。
源码里出现魔数就是待改的债。

参数化时**默认值必须与参数化前逐位一致**——"加开关"和"改行为"是两件事，要分两次做
（见 `docs/DEVELOPMENT.md` §8.3）。另外注意：**同一技能在不同地图上可能表现不同**，
技能参数要留地图级覆写的余地（见 `MapSystem` 的 `_params` 注入）。

## 3. 改了期望常量，必须在提交信息里逐项报告

用户明确要求：「随改更新被影响的期望常量，每项在提交里报告」。
提交信息里要写清**哪个常量、从什么改成什么、为什么**。没改也要写"本次未改动任何期望常量"。

提交信息用中文，把**为什么**写清楚，尤其是"原实现哪里错了"。
诊断错了要如实说——本仓库的提交历史里保留了几处"我一开始判断错了，真正原因是……"，
那是有意的，能让后来的人省下重复排查的时间。

## 4. 涉及主观判断/视觉呈现的改动，写代码前必须先跟用户确认

用户原话（连续两轮森林风格返工后）：「你写代码之前必须和我确认，写进铁律，必须
每次都遵守」——起因是"低矮的灌木丛和零星的石头"这句话本身没有歧义，但"怎么
落地成具体的几何形状、颜色、密度、排布"有大量主观判断空间，我自己连续猜了
好几版（树、树+石头、灌木+石头两种配比……），每版都在没有明确画面依据的情况下
直接写代码交付，结果每版都被打回。

**任何涉及"这东西看起来该是什么样"的改动**（美术风格、几何形状、颜色、密度、
布局这类没有唯一正确答案、只能靠具体描述或参考图收敛的东西）——写代码前，先
用文字方案 / 参考图 / 草图和用户对齐清楚要做成什么样子，得到明确认可后再动手，
不要自己脑补一个"看起来合理"的版本就直接实现再等用户反馈。**这条对所有会话、
所有类似场景都成立，不是这次森林风格这一件事的例外规则。**

不受这条约束的：需求已经给出明确、无歧义的规格（具体数值、明确的逻辑分支、
已有先例可以照抄）的实现工作——那类改动仍按用户原有的"批量做、最后验收"节奏，
不必每一步都停下来确认，否则会退化成事无巨细都要问，效率反而更差。判断某件
事算不算"主观视觉判断"本身如果也吃不准，那就先问。

---

## 常用命令

```bash
node tests/run_all.mjs                                    # 全量回归
node tools/balance_matrix.mjs --runs 20 --minutes 40      # 要下平衡结论就用这个
node tools/balance_matrix.mjs --map twisted_treeline_v1   # 换地图跑
```
